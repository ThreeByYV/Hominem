...
public:
	...
	/// Registers a named render target. The FBO is created on the first Execute() with a valid viewport.
	void CreateRenderTarget(std::string name, FramebufferFormat format);
...
private:
	...
	struct FBOEntry { Ref<Framebuffer> fbo; FramebufferFormat format; };
