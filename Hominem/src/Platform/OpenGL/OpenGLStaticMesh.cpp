...
namespace Hominem
{
    ...
    static constexpr uint32_t k_CacheVersion = 7u;          // v7: vertices baked to metres via aiProcess_GlobalScale
    ...
    std::expected<void, std::string> OpenGLStaticMesh::LoadFromFile(const std::string& path)
    {
        ...
        if (auto res = LoadAssimp(path); !res)
            return std::unexpected(res.error());
        return {};
    }

    bool OpenGLStaticMesh::LoadBinary(const std::string &binPath)
    {
    ...
    std::expected<void, std::string> OpenGLStaticMesh::LoadAssimp(const std::string &path)
    {
        ...
        // Unit conversion is handled by aiProcess_GlobalScale: the FBX importer reads
        // UnitScaleFactor and bakes cm → metres straight into the vertices. glTF/OBJ are
        // untouched (already metres). So vertices arrive in metres and node transforms
        // need no extra scaling.
        ...
        traverse = [&](const aiNode* node, const glm::mat4& parentTransform)
        {
            ...
            for (uint32_t mi = 0; mi < node->mNumMeshes; mi++)
            {
                ...
                for (uint32_t v = 0; v < mesh->mNumVertices; v++)
                {
                    ...
                    verts.push_back({
                        { p.x, p.y, p.z },  // already in metres (aiProcess_GlobalScale)
                        { n.x, n.y, n.z },
                        { u.x, u.y },
                        tangent
                    });
    ...
    void OpenGLStaticMesh::ComputeWorldAABB()
    {
    ...
    void OpenGLStaticMesh::OptimizeGeometry(std::vector<StaticVertex>& verts,
                                            std::vector<uint32_t>&     indices,
    ...
    void OpenGLStaticMesh::WriteBinary(const std::string &binPath,
    ...
    void OpenGLStaticMesh::Upload(const std::vector<StaticVertex> &verts, const std::vector<uint32_t> &indices)
    {
        ...
        glNamedBufferData(m_ModelMatrixSSBO,
            (GLsizeiptr)(maxGroups * sizeof(glm::mat4)), nullptr, GL_DYNAMIC_DRAW);
        glCreateBuffers(1, &m_DrawCommandBuffer);
        glNamedBufferData(m_DrawCommandBuffer,
        ...
        const Ref<Shader>& shader, const glm::mat4& actorTransform, const Frustum* frustum)
    {
        ...
        for (const auto& group : m_DrawGroups)
        {
            ...
            tl_batches.push_back({
                (group.Albedo         ? group.Albedo         : GetWhiteTexture()).get(),
                (group.MetalRoughness ? group.MetalRoughness : GetDefaultMetalRoughness()).get(),
                (group.NormalMap      ? group.NormalMap      : GetFlatNormalMap()).get(),
                group.IndexCount / 3
            });
        }
