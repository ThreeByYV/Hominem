...
namespace Hominem {
	...
	{
		HMN_CORE_ERROR("GLFW ERROR ({0}: {1})", error, description);
	};
	...
	void WindowsWindow::Init(const WindowProps& props)
	{
		...
		if (!s_IsGLFWInitialized)
		{
			...
			HMN_CORE_ASSERT(success, "Could not initialize GLFW!");
		...
		glfwSetKeyCallback(m_Window, [](GLFWwindow* window, int key, int scancode, int action, int mods)
			{
				...
				case GLFW_REPEAT:
				{
					//todo glfw doesn't have a repeat count, even though WIN32 API does, could implement this
