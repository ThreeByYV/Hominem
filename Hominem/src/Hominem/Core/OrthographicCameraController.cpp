...
namespace Hominem {
	...
	OrthographicCameraController::OrthographicCameraController(float aspectRatio, bool hasRotation)		                                   
		: m_AspectRatio(aspectRatio),
		  m_Camera(-m_AspectRatio * m_ZoomLevel, m_AspectRatio * m_ZoomLevel, -m_ZoomLevel, m_ZoomLevel),   //-1.78 -> 1.78,   -1 -> 1
		  m_Rotation(hasRotation)
	{
	}
	...
	bool OrthographicCameraController::OnMouseScrolled(MouseScrolledEvent& e)
	{
		...
		m_ZoomLevel = std::max(m_ZoomLevel, 0.25f); //clamp zoom to never be less than 0.25
	
		m_Camera.SetProjection(-m_AspectRatio * m_ZoomLevel, m_AspectRatio * m_ZoomLevel, -m_ZoomLevel, m_ZoomLevel);
		return false;
	}

	//Changes the aspect ratio, then recalc matrices
