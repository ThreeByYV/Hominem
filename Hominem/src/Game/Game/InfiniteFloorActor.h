...
namespace Hominem {
	...
	/// Infinite tiling floor: keeps a sliding window of physics + visual
	/// segments centred on the camera.  Old segments outside the window are
	/// destroyed so physics bodies never accumulate unboundedly.
	class InfiniteFloorActor : public Actor
	{
	public:
		// How many segments to keep on each side of the camera.
		...
		void OnUpdate(Timestep) override
		{
			...
			// Cull segments outside the window.
			float cullDist = (k_HalfWindow + 1) * m_SegW;
			auto shouldCull = [&](const Segment& s) {
				return std::abs(s.CenterX - camX) > cullDist;
			};

			for (auto& s : m_Segments)
			{
				if (shouldCull(s) && s.Body)
				{
			...
			m_Segments.erase(
				std::ranges::remove_if(m_Segments,
				                       [](const Segment& s) { return s.Body == nullptr; }).begin(),
				m_Segments.end());
		}
	...
	private:
		...
		void SpawnSegment(const float centerX)
		{
			auto world = m_Scene ? m_Scene->GetPhysicsWorld() : nullptr;
			if (!world) return;

			// Keep list sorted by X so front/back checks are O(1).
			RigidbodySpec spec;
